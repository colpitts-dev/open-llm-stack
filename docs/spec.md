...
### 5.21 Job approval watcher (plan 19)

- **The problem.** Buzz Desktop's `@mention` picker does not resolve server (`role: bot`) agents, so a human typing `@Gilfoyle approved` in a job thread routinely fails to resolve or fires an "invite them?" prompt. Independently, `BUZZ_ACP_RESPOND_TO=allowlist` with the harness's default `--subscribe mentions` means a bare, unmentioned `approved` reply never reaches the builder at all — the relay subscription itself filters out any event that does not `p`-tag the agent, before the model ever sees it.
- **Rejected alternative.** `BUZZ_ACP_SUBSCRIBE=all` on the builder was considered and rejected: one job channel carries the whole job lifecycle (ask, plan, approval, PR, review routing, verdict) in a single thread, so `all` would wake the builder on every later-stage message too, not only the approval — extra, unaddressed LLM turns competing for the single resident model's one GPU slot, plus a real risk of an unwanted reply (the base prompt still tells the model to answer any human message it sees).
- **The chosen design.** `console/approve-watch.sh` (`make approval-watch [I=<seconds>]`, foreground, Ctrl-C to stop) polls every `job-*` channel the console identity belongs to, over the same `buzz` CLI `console/post-job.sh`/`console/approve-job.sh` already use. A matching event is a threaded reply (`e`/`reply` tag) whose first word is `approved` or `#approved` (case-insensitive) from a pubkey in `TEAM_ALLOWLIST` ∪ `TEAM_SMOKE_PUBKEY` — never a teammate agent's own key. A match calls the unmodified `console/approve-job.sh <channel> <root> [note]`, which sends `approved` with a real `--mention` against the builder's pubkey, and appends the same audit-log schema (`console/audit.log`) the console's own Runner writes, with `actor:"watcher"`. `console/.watch-since` (one global poll watermark) and `console/.watch-approved` (thread-root ids already approved) are gitignored, host-local state; the same root is never re-approved even if a message lands on a watermark boundary.
- **Zero LLM turns, zero agent config touched.** No `BUZZ_ACP_*` variable changes on any agent; the watcher is a host process, like the console itself.
...
| G60 | Bare `approved` reaches the builder | a threaded, unmentioned, case-varied `approved`/`#approved` reply from an allowlisted human triggers `console/approve-job.sh` and is recorded in `console/audit.log` (`actor:"watcher"`) within one poll interval |
| G61 | Sender and content gating | a teammate agent's own `approved` reply, and non-matching content from an allowlisted human, are both ignored — no `approve-job.sh` call |
| G62 | No double-approval | a second matching `approved` reply on the same thread root does not re-fire `approve-job.sh` |
| G63 | Watcher process guard | a second `approve-watch.sh` instance refuses to start while one is already running; the first stops cleanly on `SIGTERM` |
