...
- Repo factory (plan 10): agents create repos ONLY via `agents/bin/new-repo` → `generate` from `<org>/python-template` (protection copied at birth, `block_admin_merge_override`) then self-demotion (Gitea makes the creator a repo admin; `DELETE collaborators/{self}` → team write only). Role teams `builders`/`reviewers`/`coordinators` enforce permissions (Gilfoyle cannot push; Jared cannot create repos). Bootstrap reconciles protection + strips agent collaborators on every org repo.
