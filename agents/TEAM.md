...
- Progress and deliverables, same shape for every agent:
  ...
  - Deliverables start with a bold label and are the ONLY messages that carry @mentions: `**PR:** <html_url>` then a blank line, then what changed and `@<requester> @$REVIEWER_NAME review please`; `**Review:** APPROVED|REQUEST_CHANGES|COMMENT — <one line>`; `**Score:** complexity N/5 · confidence low|medium|high — <one clause>` (the coordinator only); `**Question:** …` when you need the requester. Never use bold or a mention anywhere else.
...
- `main` is protected: CI must pass, one approval is required, and only the owner (`$GITEA_HUMAN`, or the admin `$GITEA_ADMIN`) can merge; you cannot, so never claim you did. Never push to `main`, never force-push, never modify tests to make them pass — if a test is wrong, say so in the thread.
