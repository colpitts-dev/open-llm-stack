...
## 8. Execution report (2026-09-15)

Executed against the live stack: external forge `git.colpitts.dev` (private CA via `GITEA_CA_FILE`), Docker 29.4.0, team model `ornith-max`, profiles `litellm,openwebui,buzz,buzz-agent,team`. Six runs of `scripts/validate-16.5.sh` between 00:39 and 08:21 local; runs 1–4 found real bugs (deviations 1–5 below), run 5 was the first clean pair of smokes, run 6 confirmed the `--content` fix. The log of runs 1–5 lived in `/tmp` and was lost to a host reboot at 08:00; their verdict lines survive in the session transcript and are quoted here. Run 6 log: `validate-16.5.log` in the session scratchpad (excerpts below).

| Gate | Result | Evidence |
|---|---|---|
| G47/G48 static barriers | PASS (runs 2–6) | run 6: `adversary token: issues list 404, issue #82 404, pulls 200`; `gatekeeper token: patch issue 403, read issue 200, own ledger 200`; `ledger erlich/gate: builder 404, reviewer 404, adversary 404, coordinator issues 200`; builder and adversary `-> #gate: not a channel member`; `#attack has adversary+coordinator, not gatekeeper: true; #gate has gatekeeper+coordinator, not adversary: true` |
| G45 gate script path | PASS (runs 2–6) | run 6 on demo-calc#85: `CI=success REVIEW=APPROVED APPROVALS=1 CONFORMANCE=2/2 ATTACK=COMMENT DEFECT=none SPEC=84 CRITERIA=2 VERDICT=ship REASONS=`; `deterministic=yes bad-rubric-exit=2 ledger-entries=1`. Runs 1–2 computed `no-ship` (missing conformance), run 3 onward `ship`: both branches exercised |
| G46 plan checkpoint | PASS (run 5, both smokes) | run 5 smoke 2: `plan posted after ~60s`, `acceptance checklist: 6 items`, `nothing opened before approval (PRs above #79: 0)`, `PR body links the spec issue #80`. Run 6 smoke 1 FAIL on `acceptance checklist: 2 items (want 3-6)`; see "open" below |
| G47 blind adversary | PASS (runs 3–6) | run 6: `attack review by monica: COMMENT **Attack:** 6 probes, 0 defects`, `defect label: defect/none`, `barrier: the adversary's token cannot read issues (404)`; run 5 found a real `defect/medium` on #79 and #81 |
| G48 script-decided gate | PASS (runs 3–6) | run 6: `**Verdict:** ship — demo-calc#87 (1c48e09): all checks pass · return <channel>/0`, `**Score:** complexity 1/5 · confidence high`, labels `complexity/1,confidence/high`, `ledger: erlich/gate 'demo-calc#87 ship (1c48e09)'` (`erlich/gate#19`, label `ship/yes`), `verdict equals gate-signals (ship)`, `barrier: the builder cannot see erlich/gate (404)` |
| G49 end to end | FAIL by the letter, every part passed (bar: two consecutive smoke passes + `make test` 0 in one run) | run 5: smokes `0/0`, `make test` 2 (third smoke: `**PR:**` line posted as a file path, deviation 5). Run 6: smokes `1/1` (plan format, below), `make test` 0 with `TEAM SMOKE PASS: PR #87, CI success, review APPROVED, attack defect/none, verdict ship`. Every stage of G49 has passed; never all three in one run |

Smoke PRs left unmerged on purpose (`Merge it in Gitea to close the loop`): demo-calc #79, #81, #83, #85, #87 (ornith-max), #89, #91, #93 (qwen3.8-max trial), #95, #97 (run 7).

**Run 7 on `qwen3.8-max` (08:45–08:58).** After run 6, three plain `make team-smoke` runs with the team switched by `make team-model M=qwen3.8-max`: 3/3 `TEAM SMOKE PASS` (plans in 50 s with 6, 5, 5 `- [ ]` items; push and CI milestones present each time; PRs #89 `defect/medium`, #91, #93; 13 min total). Run 7 of the validator on that model: static barriers, G45 (`CONFORMANCE=5/5 … VERDICT=ship`), G46, G47, G48 PASS; both smokes clean (`smoke=0/0`, PRs #95 and #97); `make test` exit 2 in the Open WebUI section, before the team smoke:

```
--- open-webui: chat round-trip via qwen3.6-max
curl: (28) Operation timed out after 300003 milliseconds with 0 bytes received
```

LiteLLM's ledger shows that call 11:53:02–11:58:26 UTC, `success`, 26 s past curl's `-m 300`. Ollama's log: qwen3.6-max (22 GB) had to evict the resident qwen3.8-max (`sched.go:551 … predicted to exceed available memory, evicting` at 11:53:03) while Erlich's last request was in flight on it (11:53:06–11:53:18); the qwen3.6-max load only started at 11:58:20, when qwen3.8-max's 5 m `keep_alive` expired. Same swap took 13 s at 12:02 with no team call in flight: a scheduler race in the one-slot qwen35 family (plan 14 G31), not a defect in this plan. The Open WebUI smoke picks the first model in Open WebUI's list (alphabetical, qwen3.6-max), so every `make test` swaps models there while the team may still be talking. **Deferred (one line, not applied here):** make that round-trip prefer `TEAM_MODEL` when Open WebUI lists it, as the LiteLLM round-trip already does with `SMOKE_CHAT_MODELS` (plan 14). G49 therefore stays open by the letter (never two smokes and `make test` green in one run) while every part of it has passed on qwen3.8-max.

**Team default is now `qwen3.8-max`** (`teams/piedpiper/team.toml`, `.env.example`, spec §5.8 note): 3/3 clean smokes and fewer skipped milestones, 17 GB instead of 22.

**Open: plan format from a small model.** The remaining G49 failures are all the builder's `**Plan:**` message, never the pipeline: run 6 wrote 2 `- [ ]` lines for divide_0773 (all asserts packed into one line), then `- ` lines without the box for divide_0965, then a compliant 3-line list for divide_1044. The gate itself needs only ≥1 `- [ ]` line in the spec issue (`gate-signals` line 35) and no-ships on zero, so the box form is load-bearing and the pipeline handles its absence; the 3–6 count is the smoke's bar only. Decision 2026-09-15 (Adam): stop rerunning; the mechanics are proven and a 1-in-3 formatting miss on `ornith-max` is a model property, not a defect. `make test` therefore passes when the builder follows the plan format, and prints the exact `FAIL: acceptance checklist` line when it does not.

Deviations from the plan text (each fixed in the file and mirrored here):

1. **Stale forge-team membership on role change** (`scripts/bootstrap-team.sh`, run 1: `adversary token: issues list 200`). Monica kept her old `builders` membership, whose `repo.issues:write` overrode the `adversaries` team's missing issues unit. `ensure_team` now removes members whose role no longer maps to the team.
2. **`jq` `split("\n")[0]` on an empty string** (`scripts/team-smoke.sh`, run 1: verdict/score/ledger read as `null`, stage waits ended early). Guarded; false PASS lines gone.
3. **Checklist regex** (`scripts/team-smoke.sh`, run 2: `acceptance checklist: 0 items` while the spec issue had `CRITERIA=5`). Gilfoyle wraps `- [ ]` in backticks in chat; the smoke strips backticks before counting.
4. **Reviewer posted `**Conformance:**` on the spec issue, not the PR** (`agents/roles/reviewer.md`, runs 3–4: `no **Conformance:** comment from dinesh`, confirmed by API on #73/#74). Step 4 reused `<n>` right after introducing `<s>`; the two placeholders are now named apart. Run 5 smokes both clean after this.
5. **`--content <path>` posts the path as text** (`agents/TEAM.md`, run 5 third smoke: `FAIL: PR deliverable label`, `milestone: push ABSENT`, `milestone: CI ABSENT`). Gilfoyle ran `printf … > /tmp/pr.txt && buzz messages send … --content /tmp/pr.txt` (7 of 11 sends that hour). TEAM.md now states that `--content` takes text or `-` and that a path is published verbatim. Run 6: `PR deliverable label` PASS in both smokes, push milestone present.
6. **Checklist floor** (`agents/roles/builder.md` step 1): "never fewer than three, even for a one-line change" with the three items to use. Run 6's third plan used them verbatim; its first did not (open item above).
7. **Smoke died on a zero count** (`scripts/team-smoke.sh` line 35, run 6 smoke 2: `smoke 2 exit 1` right after `plan posted`). `grep -c` exits 1 on 0 matches and `set -euo pipefail` killed the script before the FAIL line; the plan's Task 15 had `|| true`, the executed file had lost it. Restored with a comment.
8. **Waiting for `## DONE`**: `run_in_background` waiters were killed twice by the harness under "low memory" while the host had 99 GiB free; `Monitor` on the log works. The validator's own `pgrep -af '[v]alidate-16.5.sh'` also matches a launching shell whose command line quotes the script name: launch it from a launcher file (`exec bash scripts/validate-16.5.sh`) so the wrapper never carries the name.
